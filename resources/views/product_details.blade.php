    ...
    <script>
        ...
        const addToCart = () => {
            ...
            if (count != 1) {
                cuteToast({
                    type: 'info',
                    message: 'Please select colors size'
                })
                return;
            }
        ...
        <div class="flex flex-wrap md:flex-nowrap gap-6">
            ...
            <div class="shrink-0 w-full md:w-auto flex flex-col-reverse md:flex-row gap-4">
                ...
                <div class="h-96 relative  bg-white rounded-md shadow-md p-3">
                    <img id="bigImage" class="h-full aspect-[2/3]"
                         src="{{ asset('storage/' . $product->image[0]->path) }}" alt="">
            ...
            <div class="w-full flex flex-col gap-4">
                ...
                <div>
                    ...
                    <div class="flex gap-1">
                        @foreach($product->variant as $item)
                            <span onclick="selectColor('{{ $item->color->id }}')" style="background-color: {{$item->color->code}}"
                                  class="w-7 h-7 cursor-pointer rounded-full border-2">&nbsp;</span>
                    ...
                    <div class="flex gap-1 text-gray-400 text-sm">
                        @foreach($product->variant as $item)
                            <span onclick="selectSize('{{ $item->size->id }}')"
                                class=" flex justify-center  cursor-pointer items-center p-3 w-5 h-5 rounded-full border text-center border-gray-400">{{$item->size->code}}</span>
                        @endforeach
                    </div>
                ...
                <span class="bg-white shadow-md rounded-full w-8 h-8 flex items-center justify-center ">
                    ...
                    <button onclick="addToCart()" id="add_to_cart_btn"
                            class="border text-violet-600 border-violet-600 rounded w-28 text-center drop-shadow font-medium py-0.5">
                        Add
                        To Cart
                    </button>
                    <button onclick="buyNow()"
                            class="border text-white bg-violet-600 border-violet-600 rounded w-28 text-center drop-shadow font-medium py-0.5">
                        Buy
                        Now
                    </button>
                </div>
        ...
        <div>
            ...
            <div>
                <p class="text-gray-600">
                    ...
                    incidunt dolorem fugit totam non voluptatibus rerum, dicta similique assumenda sed odio. Veritatis
                    amet
                    suscipit beatae adipisci, excepturi, ab odit ipsum repudiandae quo ullam velit optio voluptas
                    possimus
                    omnis? Praesentium, nisi vitae a laborum laboriosam tempore eum voluptatum commodi, voluptatem quis
                    est
                    ut ipsum quisquam libero, tenetur nihil deleniti amet cum earum itaque perspiciatis facere! Tenetur
                    nam
